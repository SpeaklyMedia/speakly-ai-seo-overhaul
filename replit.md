...
### Deployment files
- `vercel.json` (repo root) — `"framework": null` declares a frameworkless deployment; Vercel passes raw `IncomingMessage` to all functions with no automatic body parsing. Also configures build command, output dir, per-function `maxDuration`/`memory`, SPA rewrite, and iframe-allow headers.
...
- `api/webhook.ts` (repo root) — Vercel serverless function; receives Stripe `checkout.session.completed` events. Raw body is available by default (`"framework": null`); reads body via `readRawBody` (stream events), verifies Stripe HMAC signature with `STRIPE_WEBHOOK_SECRET`, then sends payment notification email via Resend. Also includes `export const config = { api: { bodyParser: false } }` as an extra safeguard if ever run under a Next.js runtime.
